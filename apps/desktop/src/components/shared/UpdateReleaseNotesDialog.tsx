...
export function UpdateReleaseNotesDialog({ notes, onClose }: UpdateReleaseNotesDialogProps) {
  ...
  return createPortal(
    ...
    >
      ...
      />
      <motion.div
        className="relative flex max-h-[calc(100dvh-24px)] w-full max-w-2xl min-w-0 flex-col overflow-hidden rounded-26px bg-[color-mix(in_srgb,var(--bg-muted-strong)_66%,transparent)] p-1 ring-1 ring-[color-mix(in_srgb,var(--border-subtle)_84%,transparent)] sm:max-h-[calc(100dvh-32px)] sm:rounded-5xl sm:p-1.5"
      ...
      >
        <div className="flex min-h-0 flex-col overflow-hidden rounded-22px bg-[var(--surface-base)] shadow-[inset_0_1px_0_color-mix(in_srgb,var(--surface-highlight)_72%,transparent)] ring-1 ring-[color-mix(in_srgb,var(--border-subtle)_62%,transparent)] sm:rounded-3xl">
          <div className="shrink-0 p-4 pb-3 sm:p-5 sm:pb-4">
            <div className="rounded-2xl bg-[var(--bg-muted)] p-4 ring-1 ring-[var(--border-subtle)]">
              <div className="flex min-w-0 items-start gap-3 sm:gap-4">
                <span className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-[var(--success-soft)] text-[var(--success-base)] sm:size-12">
                  <CheckCircle2 className="size-6" />
                </span>
                <div className="min-w-0">
                  <span className="inline-flex max-w-full items-center rounded-full bg-[color-mix(in_srgb,var(--surface-base)_76%,transparent)] px-2.5 py-0.5 text-10px font-semibold uppercase tracking-0_2em text-[var(--text-secondary)] ring-1 ring-[var(--border-subtle)] sm:tracking-uppercase-wide">
